...
var urbandistanceWorker = (function(undefined){
    ...
    var calculateTimes = function(nextStations, lines, secondsPerKm, maxWalkTime){
        ...
        while (nsl > 0){ // as long as we have next stations to go
            for(i=0; i<nsl; i++){
                ...
                // avoid accessing keys multiple times
                ...
                */
                if (line !== -1 && stationMap[stationId] != undefined && stationMap[stationId] <= seconds){
                    for(j=0;j<srl;j++){
        ...
        */
        ...
        for(var k=0;k<fromStations.length;k++){
            ...
            if (seconds <= maxWalkTime){
                startStations.push({stationId: fromStations[k]
...
if(typeof(window) === "undefined"){
    ...
    urbandistancePoster = WorkerFacade.add("media/js/worker.js", urbandistanceWorker);
}