...
(function ($) {
    ...
    $.address = (function () {
            ...
            _parameters = function(value) {
                ...
                if (qs && qs.indexOf('=') != -1) {
                    ...
                    for (var i = 0; i < params.length; i++) {
                        ...
                        if(paramMap[kv[0]] === undefined){
                            paramMap[kv[0]] = kv[1];
                        } else{
                            if (!paramMap[kv[0]].push){
                                paramMap[kv[0]] = [paramMap[kv[0]]]
                            }
                            paramMap[kv[0]].push(kv[1]);
                        }
                    }
                }
                return paramMap;
        ...
        return {
            ...
            parameterMap: function(keyValue) {
                var i, params;
                ...
                for (i = 0; i < names.length; i++) {
                    var n = names[i],
                        v = this.parameter(n);
                    if (typeof keyValue[n] !== UNDEFINED){
                        continue;
                    }
                    if (typeof v == 'string') {
                        v = [v];
                ...
                for(var name in keyValue){
                    var value = keyValue[name];
                    params.push(name + '=' + value);
                }
                this.queryString(params.join('&'));
                return this;
            },
            updateParameters: function(add, remove){
                var i, params;
                var names = this.parameterNames();
                params = [];
                for (i = 0; i < names.length; i++) {
                    var n = names[i],
                        v = this.parameter(n);
                    if ($.inArray(n, remove) != -1){
                        continue;
                    }
                    if (typeof add[n] !== UNDEFINED){
                        continue;
                    }
                    if (typeof v == 'string') {
                        v = [v];
            ...
            deleteParameters: function(paramnames) {
                var i, params;
                var names = this.parameterNames();
                params = [];
                for (i = 0; i < names.length; i++) {
